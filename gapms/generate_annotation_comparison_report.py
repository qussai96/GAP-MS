...
def calculate_diff_bases(pred_info, ref_info):
    """Calculate the number of bases different between predicted and reference."""
    if not pred_info or not ref_info:
        return None
    ...
    pred_length = sum(end - start + 1 for start, end in pred_cds)
    ref_length = sum(end - start + 1 for start, end in ref_cds)
    
    return abs(pred_length - ref_length)
...
def generate_annotation_report(output_dir, supported_gtf, reference_gtf, 
    ...
    if all_proteins_scores.exists():
        ...
        for _, row in scores_df.iterrows():
            ...
            scores_dict[protein_id] = {
                ...
                'protein_coverage': row.get('protein_coverage', 0),
    ...
    for _, row in tmap_df.iterrows():
        ...
        if qry_id in scores_dict:
            ...
            coverage = scores_dict[qry_id].get('protein_coverage', 0)
    ...
    gtf_dir = output_dir / "Peptide_Evidence"
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python generate_annotation_comparison_report.py <output_dir> <supported_gtf> <reference_gtf> [tmap_file] [scores_file] [peptides_bed] [protein_fasta]")
        print("\nArgs:")
